...
local function addOrEditOnRule(cfg, slave)
  ...
  -- If there is an OFF rule for same item, enforce high > low
  local off_idx = indexByItem(sc.rules.off_all, item)
  if off_idx then
    local high = sc.rules.off_all[off_idx].high
    if not (high and high > low) then
      print(string.format("WARNING: existing OFF threshold for %s is %s, which is not > low %s.", item, tostring(high), tostring(low)))
...
local function addOrEditOffRule(cfg, slave)
  ...
  -- If there is an ON rule for same item, enforce high > low
  local on_idx = indexByItem(sc.rules.on_any, item)
  if on_idx then
    local low = sc.rules.on_any[on_idx].low
    if low and not (high > low) then
...
local function deleteRule(cfg, slave)
  ...
  local list
  if which == "1" then
    listName = "ON triggers (OR)"
    list = sc.rules.on_any
  elseif which == "2" then
    listName = "OFF triggers (AND)"
    list = sc.rules.off_all
  else
    return
  end

  if #list == 0 then
    print("No rules in " .. listName)
    pause()
    return
  end
...
local function selectSlave(cfg)
  ...
  if #cfg.slaves == 0 then
    print("No slaves in config. Use your slaves editor first.")
    pause()
    return nil
  end
  ...
  while true do
    ...
    if n and n >= 1 and n <= #cfg.slaves then
      return cfg.slaves[n]
    end
    print("Pick a number 0.." .. #cfg.slaves)
  end
end
...
local function slaveMenu(cfg, slave)
  while true do
    ...
    print("\nActions:")
    print("  1) Add/Edit ON trigger (OR)")
    print("  2) Add/Edit OFF trigger (AND)")
    print("  3) Delete a rule")
    print("  4) Move/Copy rules to another slave")
    print("  5) Change mode (AUTO / FORCEON / FORCEOFF)")
    print("  6) Switch slave")
    print("  0) Exit")

    local c = readLine("> ")
    if c == "1" then
      addOrEditOnRule(cfg, slave)
    elseif c == "2" then
      addOrEditOffRule(cfg, slave)
    elseif c == "3" then
      deleteRule(cfg, slave)
    elseif c == "4" then
      moveRulesToOtherSlave(cfg, slave)
    elseif c == "5" then
      cls()
      print("Set mode for " .. slave)
      print("  1) AUTO")
      print("  2) FORCEON")
      print("  3) FORCEOFF")
      local m = readLine("> ")
      local sc = cfg.slave_cfg[slave]
      if m == "1" then sc.mode = "AUTO"
      elseif m == "2" then sc.mode = "FORCEON"
      elseif m == "3" then sc.mode = "FORCEOFF" end
      saveCfg(cfg)
      pause("Saved. Press Enter")
    elseif c == "6" then
      return true -- switch
...
if not cfg then
  ...
  print("Run the master once to generate it, or create it via your slaves editor.")
  return
end
...
while true do
  local slave = selectSlave(cfg)
  if not slave then
    cls(); print("Bye.")
    break
  end
  local again = slaveMenu(cfg, slave)
  if not again then
    cls(); print("Bye.")
    break
  end
